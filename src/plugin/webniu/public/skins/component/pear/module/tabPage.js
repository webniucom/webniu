		...
		if ($(".layui-tab[lay-filter='" + this.option.elem + "'] .layui-tab-title li[lay-id]").length <=
			...
			if (opt.type === "_iframe") {
				...
				element.tabAdd(this.option.elem, {
					id: opt.id,
					title: title,
					content: `<iframe id="${opt.id}" type="${opt.type}" data-frameid="${opt.id}" scrolling="auto" frameborder="0" src="${opt.url}" style="width:100%;height:99.2%;" allowfullscreen="true"></iframe>`
				});
	...
	function createTab(option) {
		...
		$.each(option.data, function (i, item) {
			...
			if (item.type === "_iframe") {
				...
				content += `<div class="${option.index == index ? 'layui-show' : ''} layui-tab-item"><iframe id="${item.id}" type="${item.type}" data-frameid="${item.id}" scrolling="auto" frameborder="0" src="${item.url}" style="width:100%;height:99.2%;" allowfullscreen="true"></iframe></div>`
