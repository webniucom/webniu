...
layui.$(function () {
    let $ = layui.$;
    $.ajax({
        url: "/app/webniu/web/rule/permission",
        dataType: "json",
        success: function (res) {
            ...
            if (codes.indexOf('*') !== -1) {
                $("head").append("<style>*[permission]{display: inline-block !important;}</style>");
                isSuperAdmin = true;
            }
            if (self !== top) {
            ...
            if (codes.length) {
                $("head").append("<style>" + codes.join(",") + "{display: inline-block !important;}</style>");
            }
        }
    });
});