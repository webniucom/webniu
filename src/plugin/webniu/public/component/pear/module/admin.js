	...
	function(exports) {
		...
		var pearAdmin = new function() {
			...
			this.logoRender = function(param) {
				...
				$("title").html(param.base.title); //变更
			}
			...
			this.themeRender = function(option) {
				...
				var aside = localStorage.getItem("theme-aside");
				...
				localStorage.setItem("theme-aside", aside);
			...
			this.asideSkin = function(theme) { 
				var pearAdmin = $(".pear-admin .layui-side");
				pearAdmin.removeClass("double-theme");
				pearAdmin.removeClass("single-theme");
				pearAdmin.addClass(theme);
			}
		...
		function collapse() {
			...
			if (admin.is(".pear-mini")) {
				...
				//webniu-xhl 2025年3月10日13:51:13 修改
				if(aside=='double-theme'){
					$(".pear-admin .layui-side").removeClass("double-theme")
				}
		...
		body.on("click", ".setting", function() {
			...
			var headItem =
				'<li class="layui-this" data-select-header="dark-theme" >' +
				'<a href="javascript:;" data-skin="skin-blue" style="" class="clearfix full-opacity-hover">' +
			...
			headItem +=
				'<li data-select-header="light-theme" >' +
				'<a href="javascript:;" data-skin="skin-blue" style="" class="clearfix full-opacity-hover">' +
				'<div><span style="display:block; width: 20%; float: left; height: 12px; background: #28333E;"></span><span style="display:block; width: 80%; float: left; height: 12px; background: white;"></span></div>' +
				'<div><span style="display:block; width: 20%; float: left; height: 40px; background: #28333E;"></span><span style="display:block; width: 80%; float: left; height: 40px; background: #f4f5f7;"></span></div>' +
				'</a>' +
			...
			//webniu-xhl 2025年3月10日13:51:13 修改
			var styleItem =
				'<li class="layui-this" data-select-aside="double-theme" >' +
				'<a href="javascript:;" data-skin="skin-blue" style="" class="clearfix full-opacity-hover">' +
				'<div><span style="display:block; width: 5%; float: left; height: 12px; background: #28333E;"></span><span style="display:block; width: 15%; float: left; height: 12px; background: #ffffff"></span><span style="display:block; width: 80%; float: left; height: 12px; background: #f2f2f2;"></span></div>' +
				'<div><span style="display:block; width: 5%; float: left; height: 40px; background: #28333E;"></span><span style="display:block; width: 15%; float: left; height: 40px; background: #ffffff;"></span><span style="display:block; width: 80%; float: left; height: 40px; background: #f4f5f7;"></span></div>' +
				'</a>' +
				'</li>';
				 

				styleItem +=
				'<li data-select-aside="single-theme" >' +
				'<a href="javascript:;" data-skin="skin-blue" style="" class="clearfix full-opacity-hover">' +
				'<div><span style="display:block; width: 20%; float: left; height: 12px; background: #28333E;"></span><span style="display:block; width: 80%; float: left; height: 12px; background: #f2f2f2;"></span></div>' +
				'<div><span style="display:block; width: 20%; float: left; height: 40px; background: #28333E;"></span><span style="display:block; width: 80%; float: left; height: 40px; background: #f4f5f7;"></span></div>' +
				'</a>' +
				'</li>';
			...
			var styleHtml =
				'<div class="pearone-color">\n' +
				'<div class="color-title">菜单分级</div>\n' +
				'<div class="color-content">\n' +
				'<ul>\n' + styleItem + '</ul>\n' +
				'</div>\n' +
				'</div>';
			...
			layer.open({
				...
				move: false,
				content: menuHtml + headHtml + styleHtml +buildColorHtml() + moreHtml,
				success: function(layero, index) {
					...
					if (aside !== "null") {
						$("*[data-select-aside]").removeClass("layui-this");
						$("[data-select-aside='" + aside + "']").addClass("layui-this");
					}
		...
		body.on('click', '[data-select-aside]', function() {
			var theme = $(this).attr('data-select-aside');
			$('[data-select-aside]').removeClass("layui-this");
			$(this).addClass("layui-this");
			localStorage.setItem("theme-aside", theme);
			pearAdmin.asideSkin(theme);
		});
		...
		function getColorById(id) {
			var color;
			var flag = false;
			$.each(config.theme.colors, function(i, value) {
				if (value.id === id) {
					color = value;
					flag = true;
				}
