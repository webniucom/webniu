...
use plugin\webniu\app\common\Layui;
...
class TableController extends Base
{
    ...
    public function show(Request $request): Response
    {
        ...
        $offset     = ($page - 1) * $limit;
        $mysql      = config('database.connections')['plugin.webniu.mysql'];
        ...
        if ($tables) {
            ...
            foreach ($table_names as $table_name) {
                $table_rows_count[$table_name] = Util::db()->table(str_replace($prefix, '', $table_name))->count();
            }
    ...
    public function create(Request $request): Response
    {
        ...
        $data           = $request->post();
        $prefix         = config('database.connections')['plugin.webniu.mysql']['prefix'];
        ...
        Util::db()->statement("ALTER TABLE `{$prefix}$table_name` COMMENT $table_comment");
        ...
        $this->updateSchemaOption($prefix.$table_name, $form_schema_map);
    ...
    public function modify(Request $request): Response
    {
        ...
        $old_table_name_no  = str_replace($prefix, '', $old_table_name);

        $table_name     = Util::filterAlphaNum($data['table']);
        $table_name_no  = str_replace($prefix, '', $table_name);
        ...
        if ($table_name != $old_table_name) {
            Util::schema()->rename($old_table_name_no, $table_name_no);
        }
        ...
        Util::schema()->table($table_name_no, function (Blueprint $table) use ($columns, $old_columns, $keys, $table_name) {
            foreach ($columns as $column) {
                $field = $column['field'];
                // 新字段
        ...
        Util::schema()->table($table_name_no, function (Blueprint $table) use ($keys, $old_keys, $table_name) {
            foreach ($keys as $key) {
                $key_name = $key['name'];
    ...
    public function crud(Request $request): Response
    {
        ...
        $base_path = '/plugin/webniu/app';
        if ($request->method() === 'GET') {
            return raw_view("table/crud", [
                'table' => $table_name,
                'model' => "$base_path/model/$model_class.php",
                'controller' => "$base_path/web/controller/{$model_class}Controller.php",
            ]);
        }
    ...
    {
        $this->mkdir($file);
        $prefix     = $prefix = config('database.connections')['plugin.webniu.mysql']['prefix'];
        $table      = strpos($table, $prefix) === 0 ? substr($table, strlen($prefix)) : $table;
        $table_val = "'$table'";
        $pk = 'id';
        $properties = '';
        $timestamps = '';
        ...
        $columns = [];
        try {
            $database = config('database.connections')['plugin.webniu.mysql']['database'];
            //plugin.webniu.mysql
...
<html lang="zh-cn">
    <head>
        ...
        <title>新增页面</title>
        <link rel="stylesheet" href="/app/webniu/component/pear/css/pear.css" />
        <link rel="stylesheet" href="/app/webniu/component/jsoneditor/css/jsoneditor.css" />
        <link rel="stylesheet" href="/app/webniu/admin/css/reset.css" />
    </head>
    <body>
...
EOF;
    ...
    public function drop(Request $request): Response
    {
        ...
        foreach ($tables as $table) {
            Util::schema()->drop(str_replace($prefix, '', $table));
            // 删除schema
